...
[].filter.constructor('return this')()['require'] = (function (include, isIncluded, print, notify) {
	...
	const obj = function require(field, path, debug) {
		...
		try {
			directory = stack[1].match(/.*?@.*?d2bs\\(aiobot\\?.*)\\.*(\.js|\.dbj|console):/)[1].replace('\\', '/') + '/';
			filename = stack[1].match(/.*?@.*?d2bs\\aiobot\\?(.*)(\.js|\.dbj|console):/)[1];
		...
		let appendUpperFolder = !removeRelativePath(directory + field).startsWith('aiobot/libs');

		// remove the name aiobot of the file
		if (directory.startsWith('aiobot')) {
			directory = directory.substr('aiobot'.length);
		}
		...
		// remove lib again, if required in e.g. aiobot\tools but wants modules\whatever
		...
		if (!match) {
			(function () {
				...
				err.fileName = directory + myStack[2].match(/.*?@.*?d2bs\\aiobot\\?(.*)(\.js|\.dbj):/)[1];
		...
		if (!isIncluded(fullpath + '.js') && !modules.hasOwnProperty(moduleNameShort)) {
			depth && notify && print('ÿc2aiobotÿc0 ::    - loading dependency of ' + filename + ': ' + moduleNameShort);
			!depth && notify && print('ÿc2aiobotÿc0 :: Loading module: ' + moduleNameShort);
			...
			try {
				...
				if (!include(fullpath + '.js')) {
					...
					// const myStack = err.stack.match(/[^\r\n]+/g);
					// err.fileName = directory + myStack[1].match(/.*?@.*?d2bs\\aiobot\\?(.*)(\.js|\.dbj):/)[1];
...
getScript.startAsThread = function () {
	let stack = new Error().stack.match(/[^\r\n]+/g),
		filename = stack[1].match(/.*?@.*?d2bs\\aiobot\\(.*):/)[1];
