...
let [D2Bot, DataFile, ControlAction, ShitList] = (function() {
	...
	const D2Bot = {
		...
		init: function() {
			...
			const file = DataFile.getObj();
			print('Setting handle to: ' + file.handle);
			this.handle = file.handle;
			print('Set Handle to: ' + this.handle);
		},
	...
	const ControlAction = {
		...
		clickGame: function (gameName) {
			...
			let gameListControl = getControl(4, 432, 393, 160, 173);
			if (!gameListControl) {
				return false;
			}

			let text = gameListControl.getText();
			if (!text) {
				return false;
			}

			let gamesPerPage = 9; // number of visible games in scroll list
			let gameEntryHeight = Math.round(gameListControl.ysize / gamesPerPage); // height of a game entry control in scroll list
			for (let i = 0; i < text.length; i++) {
				if (text[i][0] == gameName) {
					let topListY = gameListControl.y - gameListControl.ysize; // top of list Y coord
					let midX = gameListControl.x + Math.round(gameListControl.xsize / 2); // middle list X coord
					if (i >= gameListControl.selectstart && i <= gameListControl.selectstart + gamesPerPage - 1) {
						// game is in the current scroll page, click it
						let positionInPage = i - gameListControl.selectstart;
						let y = Math.round(topListY + positionInPage * gameEntryHeight + gameEntryHeight / 2);
						gameListControl.click(midX, y);
						return true;
					} else {
						let scrollX = gameListControl.x + gameListControl.xsize + 5; // scrollbar X coord
						let topScrollY = topListY + 15; // scrollbar top Y coord, click at this Y coord to scroll up
						let bottomScrollY = gameListControl.y - 15; // scrollbar bottom Y coord, click at this Y coord to scroll down
		...
		getGameElapsedTime: function (gameName) {
			...
			let gameInfoControl = getControl(4, 609, 393, 143, 194);
			...
			let text = gameInfoControl.getText();
			...
			let timeRegExp = /\d+:\d{2}:\d{2}/i;
			for (var i = 0; i < text.length; i++) {
				let match = text[i].match(timeRegExp);
				if (match) {
					let values = match[0].split(':');
					let date = new Date(0, 0, 0, values[0], values[1], values[2], 0);
	...
	return [D2Bot, DataFile, ControlAction, ShitList];
}).call();
