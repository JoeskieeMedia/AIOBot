...
(function (module, require) {
	...
	Attack.deploy = function (unit, distance, spread, range) {
		...
		let i, grid, index, currCount,
			...
			count = 999,
			idealPos = {
				x: Math.round(Math.cos(Math.atan2(me.y - unit.y, me.x - unit.x)) * 5 + unit.x),
				y: Math.round(Math.sin(Math.atan2(me.y - unit.y, me.x - unit.x)) * 5 + unit.y)
			};
		...
		function sortGrid(a, b) {
			...
			return Math.sqrt( Math.pow((b.x - unit.x), 2) + Math.pow((b.y - unit.y), 2) ) - Math.sqrt( Math.pow((a.x - unit.x), 2) + Math.pow((a.y - unit.y), 2) );
		}
	...
	Attack.clearLevel = function (obj) {
		...
		while (rooms.length > 0) {
			...
			rooms.sort((a, b) => Math.sqrt( Math.pow((myRoom[0] - a[0]), 2) + Math.pow((myRoom[1] - a[1]), 2) ) - Math.sqrt( Math.pow((myRoom[0] - b[0]), 2) + Math.pow((myRoom[1] - b[1]), 2)));
			room = rooms.shift();
			...
			if (result) {
				...
				previousArea = result;
				if (typeof quitWhen === 'function' && quitWhen()) {
					return true;
				}
				if (!me.clear(40, spectype)) {
					break;
				}
	...
	Attack.kill = function (classId) {
		if (!classId) {
			throw TypeError('Should call Attack.Kill upon something');
		}
