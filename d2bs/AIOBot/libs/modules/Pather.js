	...
	Object.assign(Pather, {
		...
		*/
		walkTo: function (x, y, minDist = me.inTown && 2 || 4) {
			...
			while ([x, y].distance > minDist && !me.dead) {
				me.classid === 3 && me.setSkill(115, 0);
		...
		*/
		openDoors: function (x, y) {
			...
			if (!door) {
				return false;
			} // Cant open a door that we cant find
		...
		*/
		moveToExit: function (targetArea, use, clearPath) {
			...
			if (use) {
				return Array.isArray(targetArea) && me.area === targetArea[targetArea.length - 1] || me.area === targetArea;
			}
		...
		*/
		journeyTo: function (area, clearPath) {
			...
			const Town = require('Town');
			const Misc = require('Misc');
			let i, special, unit, tick, target;
